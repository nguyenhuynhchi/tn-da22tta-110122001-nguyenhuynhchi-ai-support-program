...
class renderer extends \plugin_renderer_base
{
    ...
    public function render_assign_header(assign_header $header)
    {
        ...
        if ($header->showintro || $header->activity) {
            ...
            $buttonparams = [
                'wstoken' => 'TOKEN_PLACEHOLDER',
                'wsfunction' => 'mod_assign_get_assignments',
                'moodlewsrestformat' => 'json',
                'courseids[0]' => $header->context->get_course_context()->instanceid,
                'assignmentid' => $header->assign->id,
                'cmid' => $header->coursemoduleid
            ];
            $chaturl = 'http://localhost:5173/chat?' . http_build_query($buttonparams);
            $analysisurl = 'http://localhost:5173/analysis/?' . http_build_query($buttonparams);

            $description .= '<div class="mt-3">'
                . html_writer::link($chaturl, 'Chatbot gợi ý và hỏi đáp', [
                    'class' => 'btn btn-primary',
                    'target' => '_blank',
                    'rel' => 'noopener noreferrer',
                    'id' => 'open-chatbot-suggest-btn'
                ])
                . '</div>';

            $description .= '<div class="mt-3">'
                . html_writer::link($analysisurl, 'Chatbot phân tích và sửa lỗi', [
                    'class' => 'btn btn-primary',
                    'target' => '_blank',
                    'rel' => 'noopener noreferrer',
                    'id' => 'open-chatbot-analysis-btn'
                ])
                . '</div>';
        }
